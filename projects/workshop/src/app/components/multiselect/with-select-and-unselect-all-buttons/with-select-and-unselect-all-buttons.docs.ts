...
export class SelectAndUnselectAllMenuMultiselectComponent {
  ...
  selectAll() {
    this.value = this.optionTypes.map((option) => option.value) || []; // Update the value of the combobox to reflect all selected options
  }
  clearAll() {
    this.value = [];
  }
