...
@Component({
  ...
  host: {
    '[style.display]': 'isGlobal() ? "block" : null',
    '[style.block-size]': 'isGlobal() ? "150vh" : null'
  }
})
export class ReusableBanner {
  // Unique ID for the input
  private readonly generatedId = inject(IdGenerator).newId('reusable-banner');
  ...
  readonly isGlobal = input(false, { transform: booleanAttribute });
