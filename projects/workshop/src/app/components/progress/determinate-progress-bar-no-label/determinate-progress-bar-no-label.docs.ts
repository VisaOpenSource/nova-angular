...
export class NovaDeterminateProgressBarNoLabelProgressComponent {
  ...
  readonly percentage = computed(() => Math.min(Math.trunc((this.linearProgress() / this.linearMax()) * 100), 100));
  ...
  updateLinearProgress() {
    this.linearProgress.update((val) => val + 5);
