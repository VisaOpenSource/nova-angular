...
const CHECKBOX_VALUE_ACCESSOR = {
  ...
  useExisting: forwardRef(() => ReusableCheckbox),
  multi: true
};

/** #AI-first */
@Component({
  selector: 'nova-reusable-checkbox',
  imports: [CommonModule, NovaLibModule, VisaErrorTiny],
  templateUrl: './reusable-component.docs.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [CHECKBOX_VALUE_ACCESSOR]
})
export class ReusableCheckbox extends CheckboxControlValueAccessor {
  // Unique ID for the input
  private readonly generatedId = inject(IdGenerator).newId('reusable-checkbox');
  ...
  readonly indeterminate = input(null, { transform: booleanAttribute });
  ...
  // Computed values:
  // disabled is currently broken when label is not a direct sibling of the input[v-checkbox]
  protected readonly disabled = linkedSignal(() => this.disabledInput());
  protected readonly id = computed(() => this.idInput() ?? this.generatedId);
  // This is used for the `aria-describedby` attribute on the input,
  // and should include the IDs of any elements that describe the input (e.g. description, error message)
