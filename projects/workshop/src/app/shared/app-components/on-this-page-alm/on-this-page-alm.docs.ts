...
export class OnThisPageComponent implements AfterViewInit {
  ...
  readonly items = input<{ name: string; id: string }[] | null>();
  readonly associatedTab = input<number>(0);
  ...
  constructor() {
    effect(() => {
      ...
      let scrollSubscription: any;

      if (this.workshopService.content()) {
        scrollSubscription = fromEvent(this.workshopService.content()?.nativeElement, 'scroll')
          .pipe(
            throttleTime(25),
      ...
      // Clean up subscription when effect is disposed
      return () => {
