...
/**
 * Dynamic table with linear progress indicator for loading states
 */
/** #patterns **/
@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
  selector: 'nova-workshop-dynamic-table-indeterminate-linear-progress',
  templateUrl: './indeterminate-linear-progress.docs.html',
  encapsulation: ViewEncapsulation.None,
  standalone: true,
  styleUrl: '../shared/dynamic-table.scss',
  imports: [CommonModule, NovaLibModule, ActionsButtonDynamicTableComponent]
})
export class IndeterminateLinearProgressDynamicTableComponent {
  ...
  /**
   * Whether the table is in its initial reset state
   */
  readonly reset = signal<boolean>(true);
   ...
   * Raw table data
   */
  private readonly data: RowData[] = generateBasicData(4, this.columnData);
   ...
   * Returns empty array when loading or reset
   */
  ...
  /**
   * Initiates data loading with a simulated delay
   */
  loadData() {
